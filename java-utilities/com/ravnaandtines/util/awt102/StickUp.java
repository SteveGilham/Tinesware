...
* @version 1.3 28-Dec-1998 raw AWT 1.0.2 ActionButton/Actor
* @version 1.4 3-Jan-1999 Icon control, not just big letters
*/
public class StickUp extends Dialog implements Actor {
	...
	{
		type = TYPE;
		if(TYPE == INFO) {setTitle("INFORMATION");}
		else if(TYPE == QUERY) {setTitle("QUERY");}
		else if(TYPE == ERROR) {setTitle("ERROR");}
		else {type = WARN; setTitle("WARNING");}
	}
	...
	*/
			...
			String message, String labelText, Component icon)
	{
		...
		buttons[0] = new ActionButton(labelText, this);
		decorate(message);
	}
	...
	*/
			...
			String message, String[] labelText, Component icon)
	{
		...
		{
			buttons[i] = new ActionButton(labelText[i], this);
		}
		decorate(message);
	}
	...
	*/
    ...
    /**
    * Handles window closure event by closing the JVM
