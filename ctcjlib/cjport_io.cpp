...
#define Environment(file) file->u.j.env
        ...
        case WRITEREADWIPE: 
            {
                ...
                {
                    jclass fileclass = Environment(file)->FindClass("com/ravnaandtines/ctcjava/InMemoryPortIOFile");
    ...
    static DataFileP grabDataFile(void)
    {
        ...
        {
            bug_check("out of data file handles\n");
        }
        ...
        case JAVATEXT:
            {
                ...
                std::vector<char> sink(buffer, buffer+length);
                if(eol)
                {
                    term_len = (long) strlen(file->lineEnd);
                    sink.insert(sink.end(), file->lineEnd, file->lineEnd+term_len);
                }
                sink.push_back((char)0);
...
#pragma warning (disable: 4706)
    ...
    extern "C" DataFileP vf_tempfile(long expected_size)
    {
        ...
        DataFileP result = grabDataFile();
        jclass fileclass = getEnvironment()->FindClass("com/ravnaandtines/ctcjava/InMemoryPortIOFile");
...
namespace CTCjlib {
    ...
    AbstractVirtualFile::AbstractVirtualFile() : file(NULL) {}
    
    void AbstractVirtualFile::setVFHandle(CTClib::DataFileP f)
    {
        if(!file) file = f;
    }
    AbstractVirtualFile::~AbstractVirtualFile()
    {
        if(file->u.j.file)
            Environment(file)->DeleteGlobalRef(file->u.j.file);
        file->u.j.file = NULL;
    ...
    VirtualFile::VirtualFile(JNIEnv * env, jobject jfile, bool trim)
    {
        DataFileP result = grabDataFile();
        if( result )
        {
    ...
    StringBufferVirtualFile::StringBufferVirtualFile(JNIEnv * env, jobject jfile)
    {
        DataFileP result = grabDataFile();
        if( result )
        {
