...
namespace CTClib {
    ...
    extern "C" continue_action cb_exception(cb_condition * condition)
    {
        ...
        "cbException", "(ILjava/lang/String;Lcom/ravnaandtines/ctcjava/CJPubkey;)Z");

        jint code = (condition->severity<<24)
            |(condition->module<<16)
                |(condition->code << 8)
                    |(condition->context); // assumed all in the 0-255 range
        jstring v = mbsToJstring(getEnvironment(), 
        condition->text? condition->text : "");

        // construct a CJPubkey to wrap the key with
        jlong key = (jlong)condition->pub_key;
        jclass pubkeyClass = getEnvironment()->FindClass("com/ravnaandtines/ctcjava/CJPubkey");
    ...
    extern "C" void cb_information(cb_condition * condition)
    {
        jclass fileclass = getEnvironment()->FindClass("com/ravnaandtines/ctcjava/CJGlobals");
        jmethodID jcb  = getEnvironment()->GetStaticMethodID(fileclass, 
        "cbInformation", "(ILjava/lang/String;Lcom/ravnaandtines/ctcjava/CJPubkey;)V");
        // just do it
