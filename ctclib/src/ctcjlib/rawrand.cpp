...
namespace CTClib {
    ...
    extern "C" void getRawRandom(unsigned char * data, int length)
    {
        ...
        "getRawRandom", "(I)Lcom/ravnaandtines/ctcjava/CJBytes;");
        jobject raw = getEnvironment()->CallStaticObjectMethod(fileclass, getRaw, length);

        jclass byteclass = getEnvironment()->FindClass("com/ravnaandtines/ctcjava/CJBytes");
    ...
    extern "C" boolean ensureRawRandom(int bytes)
    {
        ...
        jclass fileclass = getEnvironment()->FindClass("com/ravnaandtines/ctcjava/CJGlobals");
        jmethodID ensure = getEnvironment()->GetStaticMethodID(fileclass,
        "ensure", "(I)Z");
