 ...
 *  HISTORY
 *
 *      Mr. Tines, 1-Jan-1997: (unsigned short) coercions in send_bits
 *                 6-Apr-1997: remove crypt.h inclusion (as not used)
 *                18-Mar-2001: gzip.h has multi-line macros, so guard against
 *                             \r\n line-ends
 *                 2-Mar-2002: remove old style declarations for VC++ .NET
 *
 */
...
int length) /* number of bits */
{
    ...
    if (bi_valid > (int)Buf_size - length) {
        bi_buf |= (unsigned short)(value << bi_valid);
        put_short(bi_buf);
        bi_buf = (unsigned short)((ush)value >> (Buf_size - bi_valid));
