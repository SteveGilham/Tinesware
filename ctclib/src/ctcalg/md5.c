 ...
 */
/*
 ** Interface modified for CTC Mt.Tines Jun. 96
...
void MD5Init ( MD5_CTX **mdContext, size_t *length)
{
    ...
    *mdContext = zmalloc(*length);
    ...
    /* Load magic initialization constants.
           */
    (*mdContext)->buf[0] = (uint32_t)0x67452301L;
    (*mdContext)->buf[1] = (uint32_t)0xefcdab89L;
    (*mdContext)->buf[2] = (uint32_t)0x98badcfeL;
    (*mdContext)->buf[3] = (uint32_t)0x10325476L;
}
 ...
 */
void MD5Update (register MD5_CTX *mdContext, byte *inBuf, uint32_t inLen)
{
    register int i, ii;
    int mdi;
    uint32_t in[16];
 ...
 */
void MD5Final (MD5_CTX **pmdContext, byte digest[MD5HASHSIZE], size_t length)
{
    MD5_CTX *mdContext = *pmdContext;
    uint32_t in[16];
    int mdi;
    unsigned int i, ii;
    unsigned int padLen;
    ...
    for (i = 0, ii = 0; i < 4; i++, ii += 4) {
        digest[ii] = (byte) (mdContext->buf[i] & 0xFF);
        digest[ii+1] = (byte)((mdContext->buf[i] >> 8) & 0xFF);
        digest[ii+2] = (byte)((mdContext->buf[i] >> 16) & 0xFF);
        digest[ii+3] = (byte)((mdContext->buf[i] >> 24) & 0xFF);
    }

    if(length > 0) zfree((void**)pmdContext, length);
}
 ...
 */
void Transform(register uint32_t *buf,register uint32_t *in)
{
