/*	more.c  - Unix-style "more" paging output for PGP.
   ...
   Modified to CTC PORT_IO interface and stripped down -- Mr. Tines 14-Mar-1998
*/
...
* Routine more_file()
* Args
*         DataFileP    inFile       file to output
* Return
*       	 int
* Comment
* Blort a file to the screen with page breaks, intelligent handling of line
   ...
   chars
*/
int more_file(DataFileP inFile)
{
	int lines = 0,i;
	long fileLen;
	char *p;
   ...
   || (!isatty(fileno(stdout)))
	...
	{
		vf_setpos(inFile,   0);
		while((length = vf_readline(buffer, 256, inFile)) >= 0)
