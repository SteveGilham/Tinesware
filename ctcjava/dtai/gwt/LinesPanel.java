 ...
 */
class LinesPanel extends PanelGadget implements Runnable {
    ...
    private TextAreaGadget textarea;
     ...
     * @param textarea - TBD
     */
    public LinesPanel(TextAreaGadget textarea) {
    ...
    public String getSelectedText() {
        ...
        for (int i = 0; i < gadgetCount; i++) {
            ...
            if (selText.length() > 0 ) {
                if ((firstTime) && (selectionEnd > selectionStart+selText.length())) {
                    wholeLine = true;
                    firstTime = false;
                }
                ...
                text += selText;
                if ((line.getLineBreak()) && wholeLine) {
                    text += "\n";
                }
            }
        }
    ...
    public void insert(String str, int pos) {
        ...
        if ((extraText != null) && (currentLine != null)) {
            currentLine.select(currentLine.length()-1);
...
// THIS FUNCTION IS still a little SLOW!!!
	...
	protected void paste() {
	    ...
	    deleteSelection();
        insert(GadgetClipboard.getText(), selectionStart);
        repaint();
	}
