 ...
 */
class TextLine extends DisplayGadget {
    ...
    public Dimension getPreferredSize() {
	    ...
	    pref_width = metrics.stringWidth(str)+2;
        if (metrics instanceof SpecialFontMetrics) {
            pref_height = ((SpecialFontMetrics)metrics).stringAscent(str) +
	...
	public void setText( String text ) {
		synchronized(getTreeLock()) {
    	    ...
    	    if ( ( this.text == null ) ||
                ...
                this.text = new StringBuffer(text.length());
    ...
    public void replaceText(String str, int start, int end) {
	    ...
	    text = new StringBuffer( string.substring(0, start) );
	    text.append( str );
	    text.append( string.substring(end) );
    }
    ...
    private void paintSelection( GadgetGraphics g, Color fg, int x, int y, int text_y,
		...
		if ( selectionStart < selectionEnd ) {
		    FontMetrics metrics = getFontMetrics(getFont());
		    int start_x = x + metrics.stringWidth(
                    draw_string.substring( 0, selectionStart ) );
		    int end_x = x + metrics.stringWidth(
                    draw_string.substring( 0, selectionEnd ) );
			...
			g.drawString( draw_string.substring( selectionStart, selectionEnd ), start_x, text_y );
		}
	}
    ...
    protected void paintForeground( GadgetGraphics g,
		...
		if ( fg != null ) {
			...
			if ( echoCharIsSet() ) {
			    StringBuffer echo = new StringBuffer( text.length() );
			    for ( int i = 0; i < text.length(); i++ ) {
			        echo.append( echoChar );
			    }
			    draw_string = echo.toString();
			}
			else {
			    draw_string = getText();
			}
			...
			int ascent;
            if (metrics instanceof SpecialFontMetrics) {
                ascent = ((SpecialFontMetrics)metrics).stringAscent(draw_string);
            } else {
        	...
        	int string_width = metrics.stringWidth( draw_string );
 			int pixels_to_cursor = metrics.stringWidth( draw_string.substring( 0, caretPosition ) );
			...
			g.setColor(fg);
			g.drawString( draw_string, x + 1 /* room for cursor */, text_y );
	...
	protected int findTextPos( int x ) {
        ...
        for ( i = 0; i < str.length(); i++ ) {
            int next_x = metrics.stringWidth( str.substring( 0, i+1 ) );
	...
	protected int findCaretPosition( int pos ) {
        ...
        return metrics.stringWidth( str.substring( 0, pos ) );
	}
	...
	protected Dimension getForegroundSize( GadgetGraphics g, int columns ) {
	    ...
	    String str = text.toString();
        if (metrics instanceof SpecialFontMetrics) {
            fg.height = ((SpecialFontMetrics)metrics).stringAscent(str) +
                          metrics.getDescent() + metrics.getLeading();
        } else {
    		fg.height = metrics.getHeight();
        }
		if (columns == 0) {
		    fg.width = metrics.stringWidth(getText());
		}
		else {
