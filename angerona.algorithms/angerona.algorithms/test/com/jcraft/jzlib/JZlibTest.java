 ...
 * and open the template inStr the editor.
 */
 ...
 * @author Steve
 */
public class JZlibTest extends TestCase { //NOPMD complex

    public JZlibTest(final String testName) {
        super(testName);
    }
    private static final byte[] dictionary = "hello ".getBytes();
    private static final byte[] hello = "hello, hello! ".getBytes();
    ...
    private void checkErr(final ZStream zStr, final int err, final String msg) {
        String message = (null == zStr.msg) ? "" : zStr.msg + " ";
        message += msg + " error: " + err; //NOPMD
        assertEquals(message, JZlib.Z_OK, err);
    }

    public void testInflateDeflate() { //NOPMD complex
        int err;
        final int comprLen = 40000;
        final int uncomprLen = comprLen;
        final byte[] compr = new byte[comprLen];
        final byte[] uncompr = new byte[uncomprLen];

        final ZStream c_stream = new ZStream();

        err =
                c_stream.deflateInit(JZlib.Z_DEFAULT_COMPRESSION);
        checkErr(c_stream, err, "deflateInit");

        c_stream.nextIn = hello;
        c_stream.nextInIndex = 0;

        c_stream.nextOut = compr;
        c_stream.nextOutIndex = 0;

        while (c_stream.totalIn != hello.length &&
                c_stream.totalOut < comprLen) {
            c_stream.availIn = c_stream.availOut = 1; // force small buffers
            err = c_stream.deflate(JZlib.Z_NO_FLUSH);
            checkErr(c_stream, err, "deflate 1");
        }

        while (true) {
            c_stream.availOut = 1;
            err =
                    c_stream.deflate(JZlib.Z_FINISH);
            if (err == JZlib.Z_STREAM_END) {
                break;
            }

            checkErr(c_stream, err, "deflate 2");
        }

        err = c_stream.deflateEnd();
        checkErr(c_stream, err, "deflateEnd 1");

        final ZStream d_stream = new ZStream();
        ...
        checkErr(d_stream, err, "inflateInit 1");
        ...
        while (d_stream.totalOut < uncomprLen &&
            ...
            checkErr(d_stream, err, "inflate");
        }

        err = d_stream.inflateEnd();
        checkErr(d_stream, err, "inflateEnd 1");

        int index1 = 0;
        for (; index1 < hello.length; index1++) {
            if (hello[index1] == 0) {
                break;
            }
        ...
        int index2 = 0;
        for (; index2 < uncompr.length; index2++) {
            if (uncompr[index2] == 0) {
                break;
            }
        ...
        if (index1 == index2) {
            for (index1 = 0; index1 < index2; index1++) {
                if (hello[index1] != uncompr[index1]) {
                    break;
                }

            }
        }
        assertEquals("bad inflate", index1, index2);
    }
    
    
 public void testDictDeflateInflate()
 {
    int err;
    final int comprLen=40000;
    final int uncomprLen=comprLen;
    ...
    checkErr(c_stream, err, "deflateSetDictionary");

    dictId=c_stream.adler;

    c_stream.nextOut = compr;
    c_stream.nextOutIndex = 0;
    c_stream.availOut = comprLen;

    c_stream.nextIn = hello;
    c_stream.nextInIndex = 0;
    c_stream.availIn = hello.length;

    err = c_stream.deflate(JZlib.Z_FINISH);
    assertEquals("deflate should report Z_STREAM_END", JZlib.Z_STREAM_END, err);

    
    err = c_stream.deflateEnd();
    checkErr(c_stream, err, "deflateEnd");
    ...
    while(true){
      ...
      if(err==JZlib.Z_NEED_DICT){
          assertEquals("unexpected dictionary", (int)dictId, (int)d_stream.adler);

	err=d_stream.inflateSetDictionary(dictionary, dictionary.length);
      }
      checkErr(d_stream, err, "inflate with dict");
    }    
    ...
    int index=0;
    for(;index<uncompr.length; index++){ if(uncompr[index]==0) {break;}}

    assertEquals("after inflateSync():", 
            "hello, hello!", new String(uncompr, 0, index));
  }
   ...
   public void testFlushSync(){
    ...
    final byte[] uncompr=new byte[uncomprLen];
    final int len = hello.length;

    final ZStream c_stream=new ZStream();

    err=c_stream.deflateInit(JZlib.Z_DEFAULT_COMPRESSION);
    checkErr(c_stream, err, "deflate 3");
    ...
    checkErr(c_stream, err, "deflate 4");

    compr[3]++;              // force an error inStr first compressed block
    c_stream.availIn=len-3;

    err = c_stream.deflate(JZlib.Z_FINISH);
    if(err!=JZlib.Z_STREAM_END){
      checkErr(c_stream, err, "deflate 5");
    }
    err = c_stream.deflateEnd();
    checkErr(c_stream, err, "deflateEnd");
    comprLen=(int)(c_stream.totalOut);
    ...
    checkErr(d_stream, err, "inflateSync");

    err=d_stream.inflate(JZlib.Z_FINISH);
        /* Because of incorrect adler32 */
    assertEquals("inflate should report DATA_ERROR", JZlib.Z_DATA_ERROR, err);
    ...
    assertEquals("after inflateSync():", 
            "lo, hello!", new String(uncompr, 0, index));
  }
  ...
  public void testLargeDeflateInflate() {
    ...
    // very well:
    c_stream.nextIn=uncompr;
    c_stream.availIn=uncomprLen;
    err=c_stream.deflate(JZlib.Z_NO_FLUSH);
    checkErr(c_stream, err, "deflate");
    assertEquals("deflate not greedy", 0, c_stream.availIn);


    // Feed inStr already compressed data and switch to no compression:
    c_stream.deflateParams(JZlib.Z_NO_COMPRESSION, JZlib.Z_DEFAULT_STRATEGY);
    c_stream.nextIn=compr;
    c_stream.nextInIndex=0;
    c_stream.availIn=comprLen/2;
    err=c_stream.deflate(JZlib.Z_NO_FLUSH);
    checkErr(c_stream, err, "deflate");
    ...
    while(true){
      ...
      checkErr(d_stream, err, "inflate large");
    }

    err=d_stream.inflateEnd();
    checkErr(d_stream, err, "inflateEnd");

    assertEquals("bad large inflate: "+d_stream.totalOut,
            2*uncomprLen + comprLen/2,
            d_stream.totalOut);

  }
  public void testStreamDeflateInflate() throws Exception {
      final ByteArrayOutputStream out=new ByteArrayOutputStream();
      final ZOutputStream zOut=new ZOutputStream(out, JZlib.Z_BEST_COMPRESSION);
      final ObjectOutputStream objOut=new ObjectOutputStream(zOut);
      final String input="Hello World!";
      objOut.writeObject(input);
      zOut.close();

      final ByteArrayInputStream inStr=new ByteArrayInputStream(out.toByteArray());
      final ZInputStream zIn=new ZInputStream(inStr);
      final ObjectInputStream objIn=new ObjectInputStream(zIn);
      assertEquals(input, input, objIn.readObject().toString());
  }

    public void testInterop1()  throws Exception
    {
        final ByteArrayOutputStream out = new ByteArrayOutputStream();
        final ZOutputStream zOut = new ZOutputStream(out, JZlib.Z_BEST_COMPRESSION);
        final ObjectOutputStream objOut = new ObjectOutputStream(zOut);
        final String input = magnaCarta;
        objOut.writeObject(input);
        zOut.close();

        final ByteArrayInputStream inStr = new ByteArrayInputStream(out.toByteArray());
        final java.util.zip.InflaterInputStream zIn =
            new java.util.zip.InflaterInputStream(inStr);
        final ObjectInputStream objIn = new ObjectInputStream(zIn);
        assertEquals(input, input, objIn.readObject().toString());
    }

    public void testInterop2()  throws Exception
    {
        final ByteArrayOutputStream out = new ByteArrayOutputStream();
        final java.util.zip.DeflaterOutputStream zOut=
    ...
    public void testInterop3()  throws Exception
    {
        final ByteArrayOutputStream out = new ByteArrayOutputStream();
        final ZOutputStream zOut = new ZOutputStream(out, 
            JZlib.Z_BEST_COMPRESSION, 13, false);
    ...
    /* Magna Carta text at http://www.constitution.org/eng/magnacar.htm
    ...
    */
    ...
    private static final String magnaCarta =
